...
- **Local customizations so far**:
  ...
  - **i18n overlay** (Chinese translation completion): overrides live in `app/javascript/dashboard/i18n/custom/` (frontend) and `customizations/i18n/backend/` (backend + super admin) — both new dirs upstream doesn't have. NEVER edit upstream `zh_CN` locale files directly. Hooks (marked `CUSTOM-I18N-HOOK`): a 3-line patch in `app/javascript/dashboard/i18n/index.js`, plus `config/initializers/zz_custom_i18n.rb` (also forces the super admin panel to administrate's `zh-CN` locale). After every upstream sync run `bundle exec rake custom_i18n:verify` (hook integrity) and `custom_i18n:check` (new untranslated keys). NOTE: the overrides dir must not be named `custom/` at repo root — that's a reserved ChatwootApp extension root and breaks boot.
