...
- **Local customizations so far**:
  ...
  - **i18n overlay** (Chinese translation completion): overrides live in `app/javascript/dashboard/i18n/custom/` (frontend) and `customizations/i18n/backend/` (backend + super admin) — both new dirs upstream doesn't have. NEVER edit upstream `zh_CN` locale files directly. Hooks (marked `CUSTOM-I18N-HOOK`): a 3-line patch in `app/javascript/dashboard/i18n/index.js`; `config/initializers/zz_custom_i18n.rb` (forces super admin + its devise sign-in to administrate's `zh-CN` locale); small `t(..., default:)` patches in super admin ERB views (`application/index.html.erb`, `application/_navigation.html.erb`, `application/_settings_menu.html.erb`, `devise/sessions/new.html.erb`) — all degrade gracefully to upstream English if a key is missing. After every upstream sync run `bundle exec rake custom_i18n:verify` (hook integrity) and `custom_i18n:check` (new untranslated keys). NOTE: the overrides dir must not be named `custom/` at repo root — that's a reserved ChatwootApp extension root and breaks boot.
